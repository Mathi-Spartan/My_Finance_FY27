...
/* ---------- the deeper read ---------- */
export function deepAnalyse(rows, a) {
  const spend = (a.excluded ? rows.filter((r) => !isSelfTransfer(r.description)) : rows)
    .filter((r) => r.direction === 'out');
  const income = (a.excluded ? rows.filter((r) => !isSelfTransfer(r.description)) : rows)
    .filter((r) => r.direction === 'in');

  // --- concentration: how much of the total sits in how few places ---
  const sorted = spend.slice().sort((x, y) => y.amount - x.amount);
  const shareOfTop = (n) => (a.totalOut ? sum(sorted.slice(0, n)) / a.totalOut * 100 : 0);
  const payees = a.byMerchant;
  let acc = 0, payeesFor80 = 0;
  for (const m of payees) { acc += m.total; payeesFor80++; if (acc >= a.totalOut * 0.8) break; }

  // --- volatility of daily spending ---
  const daily = a.byDay.map((d) => d.total);
  const mean = daily.length ? daily.reduce((s, x) => s + x, 0) / daily.length : 0;
  const sd = daily.length
    ? Math.sqrt(daily.reduce((s, x) => s + (x - mean) ** 2, 0) / daily.length) : 0;

  // --- where in the month the money goes ---
  const weekOfMonth = [
    { key: '1st–7th', total: 0, count: 0 },
    { key: '8th–14th', total: 0, count: 0 },
    { key: '15th–21st', total: 0, count: 0 },
    { key: '22nd–end', total: 0, count: 0 },
  ];
  spend.forEach((r) => {
    const d = r.date.getDate();
    const i = d <= 7 ? 0 : d <= 14 ? 1 : d <= 21 ? 2 : 3;
    weekOfMonth[i].total += r.amount; weekOfMonth[i].count++;
  });

  // --- weekday against weekend ---
  const weekend = spend.filter((r) => [0, 6].includes(r.date.getDay()));
  const weekday = spend.filter((r) => ![0, 6].includes(r.date.getDay()));

  // --- runs of days with and without spending ---
  const spentSet = new Set(spend.map((r) => r.iso));
  let runSpend = 0, bestSpend = 0, runQuiet = 0, bestQuiet = 0;
  for (let t = a.from.getTime(); t <= a.to.getTime(); t += 86400000) {
    const iso = new Date(t).toISOString().slice(0, 10);
    if (spentSet.has(iso)) { runSpend++; runQuiet = 0; bestSpend = Math.max(bestSpend, runSpend); }
    else { runQuiet++; runSpend = 0; bestQuiet = Math.max(bestQuiet, runQuiet); }
  }

  // --- who is new and who keeps coming back ---
  const once = payees.filter((m) => m.count === 1);
  const regular = payees.filter((m) => m.count >= 5);

  // --- first half against second half of the window ---
  const mid = new Date((a.from.getTime() + a.to.getTime()) / 2);
  const firstHalf = sum(spend.filter((r) => r.date <= mid));
  const secondHalf = sum(spend.filter((r) => r.date > mid));

  // --- same payee, same amount, same day: worth a second look ---
  const seen = {};
  const dupes = [];
  spend.forEach((r) => {
    const k = `${r.iso}|${merchantOf(r.description)}|${r.amount}`;
    seen[k] = (seen[k] || 0) + 1;
    if (seen[k] === 2) dupes.push({ iso: r.iso, name: merchantOf(r.description), amount: r.amount });
  });

  // --- round numbers usually mean transfers, odd ones mean purchases ---
  const round = spend.filter((r) => r.amount >= 1000 && r.amount % 500 === 0);

  // --- debt, broken down by who is owed ---
  const debtRows = spend.filter((r) => /Loans & EMI|Credit card/.test(categorise(r.description)));
  const byLender = {};
  debtRows.forEach((r) => {
    const k = merchantOf(r.description);
    if (!byLender[k]) byLender[k] = { key: k, total: 0, count: 0 };
    byLender[k].total += r.amount; byLender[k].count++;
  });

  // --- income: what is salary and what is not ---
  const salary = income.filter((r) => /sal |salary|dkm online|dayforce/i.test(r.description));
  const otherIn = income.filter((r) => !salary.includes(r));

  const debtRatio = a.totalIn > 0 ? (a.debt.total / a.totalIn) * 100 : null;
  const savingsRate = a.totalIn > 0 ? ((a.totalIn - a.totalOut) / a.totalIn) * 100 : null;
  const burnPerDay = a.totalOut / a.days;
  const runway = burnPerDay > 0 && a.closingBalance !== null ? a.closingBalance / burnPerDay : null;

  return {
    top5Share: shareOfTop(5), top10Share: shareOfTop(10), top20Share: shareOfTop(20),
    payeesFor80, payeeCount: payees.length,
    onceOnly: once.length, regulars: regular.length,
    dailyMean: mean, dailySd: sd,
    volatility: mean > 0 ? (sd / mean) * 100 : 0,
    weekOfMonth,
    weekend: { total: sum(weekend), count: weekend.length },
    weekday: { total: sum(weekday), count: weekday.length },
    longestSpendRun: bestSpend, longestQuietRun: bestQuiet,
    firstHalf, secondHalf,
    trend: firstHalf > 0 ? ((secondHalf - firstHalf) / firstHalf) * 100 : null,
    dupes: dupes.slice(0, 6), dupeCount: dupes.length,
    roundCount: round.length, roundTotal: sum(round),
    byLender: Object.values(byLender).sort((x, y) => y.total - x.total),
    salary: { total: sum(salary), count: salary.length },
    otherIn: { total: sum(otherIn), count: otherIn.length },
    debtRatio, savingsRate, burnPerDay, runway,
  };
}


/* ============================================================
   Instrumentation for the interactive views
   ============================================================ */

// Balance on every day in the window, carried forward across quiet days.
export function balanceSeries(rows) {
  const byDay = {};
  rows.forEach((r) => { if (r.balance !== null) byDay[r.iso] = r.balance; });
  ...
  for (let t = from.getTime(); t <= to.getTime(); t += 86400000) {
    const iso = new Date(t).toISOString().slice(0, 10);
    if (byDay[iso] !== undefined) last = byDay[iso];
    out.push({ iso, balance: last });
  }
  return out;
}

// Spending per day across the whole window, including the zeroes.
export function dailySeries(rows, from, to) {
  const spend = {};
  const count = {};
  rows.filter((r) => r.direction === 'out').forEach((r) => {
    spend[r.iso] = (spend[r.iso] || 0) + r.amount;
    count[r.iso] = (count[r.iso] || 0) + 1;
  });
  const out = [];
  for (let t = from.getTime(); t <= to.getTime(); t += 86400000) {
    const iso = new Date(t).toISOString().slice(0, 10);
    out.push({ iso, total: spend[iso] || 0, count: count[iso] || 0 });
  }
  return out;
}

// Category against month, for a heatmap.
export function categoryMatrix(rows) {
  const spend = rows.filter((r) => r.direction === 'out');
  const months = [...new Set(spend.map((r) => r.iso.slice(0, 7)))].sort();
  const cats = {};
  spend.forEach((r) => {
    const c = categorise(r.description);
    const m = r.iso.slice(0, 7);
    cats[c] = cats[c] || { key: c, total: 0, byMonth: {} };
    cats[c].total += r.amount;
    cats[c].byMonth[m] = (cats[c].byMonth[m] || 0) + r.amount;
  });
  const rowsOut = Object.values(cats).sort((a, b) => b.total - a.total);
  const peak = Math.max(...rowsOut.flatMap((c) => months.map((m) => c.byMonth[m] || 0)), 1);
  return { months, rows: rowsOut, peak };
}

// How payment sizes are distributed, on a log scale.
const BANDS = [
  ['Under ₹100', 0, 100], ['₹100–500', 100, 500], ['₹500–1k', 500, 1000],
  ['₹1k–5k', 1000, 5000], ['₹5k–20k', 5000, 20000], ['₹20k–1L', 20000, 100000],
  ['Over ₹1L', 100000, Infinity],
];
export function sizeBands(rows) {
  const spend = rows.filter((r) => r.direction === 'out');
  const total = spend.reduce((s, r) => s + r.amount, 0);
  return BANDS.map(([key, lo, hi]) => {
    const items = spend.filter((r) => r.amount >= lo && r.amount < hi);
    const t = items.reduce((s, r) => s + r.amount, 0);
    return { key, count: items.length, total: t, share: total ? (t / total) * 100 : 0, items };
  }).filter((b) => b.count > 0);
}

// Everything known about one payee.
export function payeeProfile(rows, name) {
  const items = rows.filter((r) => merchantOf(r.description) === name)
    .sort((a, b) => a.date - b.date);
  const out = items.filter((r) => r.direction === 'out');
  const total = out.reduce((s, r) => s + r.amount, 0);
  const amounts = out.map((r) => r.amount);
  const gaps = [];
  for (let i = 1; i < items.length; i++) gaps.push((items[i].date - items[i - 1].date) / 86400000);
  const avgGap = gaps.length ? gaps.reduce((s, g) => s + g, 0) / gaps.length : null;
  return {
    name, items, count: out.length, total,
    avg: out.length ? total / out.length : 0,
    min: amounts.length ? Math.min(...amounts) : 0,
    max: amounts.length ? Math.max(...amounts) : 0,
    first: items[0]?.date || null,
    last: items[items.length - 1]?.date || null,
    avgGap,
    category: items.length ? categorise(items[0].description) : 'Other',
    nextExpected: avgGap && items.length >= 3
      ? new Date(items[items.length - 1].date.getTime() + avgGap * 86400000) : null,
  };
}

// Where the month is heading if the rest of it looks like what came before.
export function projection(rows, a) {
  const months = a.byMonth.slice().sort((x, y) => x.key.localeCompare(y.key));
  ...
  const projected = (last.total / daysElapsed) * daysInMonth;
  const prior = months.slice(0, -1);
  const priorAvg = prior.length ? prior.reduce((s, m) => s + m.total, 0) / prior.length : null;
  return {
    month: lastKey, soFar: last.total, projected, daysElapsed, daysInMonth,
    priorAvg,
    vsPrior: priorAvg ? ((projected - priorAvg) / priorAvg) * 100 : null,
  };
}
