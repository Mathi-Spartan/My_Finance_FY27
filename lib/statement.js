...
export function parseStatement(text) {
  ...
  for (const r of rows(text)) {
    ...
    // The last figure on a statement line is almost always the running balance;
    // the one before it is the transaction. With only one figure, that's it.
    let amount, balance = null;
    if (amts.length >= 2) {
      balance = amts[amts.length - 1].value;
      amount = amts[amts.length - 2].value;
    } else {
      amount = amts[0].value;
    }
    if (amount <= 0) { skipped.push(r.text.slice(0, 90)); continue; }

    const hasCr = CR.test(r.text);
    const hasDr = DR.test(r.text);
    let direction = hasCr && !hasDr ? 'in' : hasDr && !hasCr ? 'out' : null;

    // No wording either way: a rising balance means money came in.
    if (!direction && balance !== null && parsed.length) {
      const prev = parsed[parsed.length - 1].balance;
      if (prev !== null) direction = balance > prev ? 'in' : 'out';
    }
    if (!direction) direction = hasCr ? 'in' : 'out';
