...
/* ---------- experiments over money, now that health is gone ----------
   Turns transactions into one row per day so the same experiment machinery
   works: spend_out is what left that day, entries is how many you logged. */
export function daysFromTxs(txs, from, to) {
  const byDay = {};
  (txs || []).forEach((t) => {
    const iso = String(t.occurred_at).slice(0, 10);
    if (!byDay[iso]) byDay[iso] = { on_date: iso, spend_out: 0, money_in: 0, entries: 0 };
    const amt = Number(t.amount) || 0;
    if (t.direction === 'out') byDay[iso].spend_out += amt;
    else byDay[iso].money_in += amt;
    byDay[iso].entries += 1;
  });

  // fill the gaps: a day with no entries is a real zero, not missing data
  const out = [];
  const start = new Date(from + 'T12:00:00');
  const end = new Date(to + 'T12:00:00');
  for (let t = start.getTime(); t <= end.getTime(); t += 86400000) {
    const iso = new Date(t).toISOString().slice(0, 10);
    out.push(byDay[iso] || { on_date: iso, spend_out: 0, money_in: 0, entries: 0 });
  }
  return out;
}

export const MONEY_METRICS = {
  spend_out: { key: 'spend_out', label: 'Spent that day', unit: '₹', better: 'down' },
  money_in:  { key: 'money_in',  label: 'Came in that day', unit: '₹', better: 'up' },
  entries:   { key: 'entries',   label: 'Entries logged', unit: '', better: 'up' },
};
