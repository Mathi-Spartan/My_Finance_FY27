...
// Balance walked backwards from today, so the hero can show where it has been
// rather than only where it is.
export function balanceTrend(txs, accounts, days = 30) {
  const live = (accounts || []).filter((a) => !a.archived && !isCard(a));
  const ids = new Set(live.map((a) => a.id));
  const bal = accountBalances(txs, accounts);
  let running = live.reduce((s, a) => s + n2(bal[a.id] || 0), 0);

  const out = [];
  const today = new Date(); today.setHours(12, 0, 0, 0);
  const byDay = {};
  (txs || []).forEach((t) => {
    if (!ids.has(t.account_id)) return;
    const iso = String(t.occurred_at).slice(0, 10);
    const amt = n2(t.amount) * (t.direction === 'in' ? 1 : -1);
    byDay[iso] = (byDay[iso] || 0) + amt;
  });

  for (let i = 0; i < days; i++) {
    const d = new Date(today.getTime() - i * 86400000);
    const iso = d.toISOString().slice(0, 10);
    out.unshift({ iso, value: running });
    running -= byDay[iso] || 0;      // step back over that day's movement
  }
  return out;
}
