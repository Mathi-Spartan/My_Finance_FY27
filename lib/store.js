...
export function StoreProvider({ children }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [a, c, t, r, s, ap, dr, tp, sal, ln, lp, hd, ex, el, hk] = await Promise.all([
        ...
        q(() => supabase.from('health_days').select('*').order('on_date')),
        q(() => supabase.from('experiments').select('*').order('created_at')),
        q(() => supabase.from('health_days').select('*').order('on_date', { ascending: false }).limit(400)),
        q(() => supabase.from('experiments').select('*').order('created_at', { ascending: false })),
        q(() => supabase.from('experiment_logs').select('*')),
        q(() => supabase.from('health_tokens').select('*').order('created_at', { ascending: false })),
      ]);
      ...
      setHealthDays(hd.data || []);
      setExperiments(ex.data || []);
      setExperimentLogs(el.data || []);
      setSyncKeys(hk.data || []);
  ...
  const makeSyncKey = async (label = 'iPhone') => {
    // long random string, generated on the device, never derived from anything
    const bytes = new Uint8Array(24);
    crypto.getRandomValues(bytes);
    const token = [...bytes].map((b) => b.toString(36).padStart(2, '0')).join('').slice(0, 40);
    const { data, error } = await supabase.from('health_tokens')
      .insert({ token, user_id: uid, label }).select().single();
    if (error) { say(error.message); return null; }
    setSyncKeys((p) => [data, ...p]);
    return data;
  };

  const revokeSyncKey = async (token) => {
    const { error } = await supabase.from('health_tokens').delete().eq('token', token);
    if (error) { say(error.message); return; }
    setSyncKeys((p) => p.filter((k) => k.token !== token));
    say('Key revoked');
  };

  const saveHealthDay = async (row) => {
    const { data, error } = await supabase.from('health_days')
      .upsert({ ...row, user_id: uid, updated_at: new Date().toISOString() },
              { onConflict: 'user_id,on_date' })
      .select().single();
    if (error) { say(error.message); return null; }
    setHealthDays((p) => [data, ...p.filter((d) => d.on_date !== data.on_date)]
      .sort((a, b) => b.on_date.localeCompare(a.on_date)));
    say('Reading saved');
    return data;
  };

  const addExperiment = async (row) => {
  ...
  const deleteExperiment = async (id) => {
    ...
    setExperimentLogs((p) => p.filter((l) => l.experiment_id !== id));
    say('Experiment deleted');
  };

  const logExperiment = async (experimentId, onDate, kept) => {
    const { data, error } = await supabase.from('experiment_logs')
      .upsert({ user_id: uid, experiment_id: experimentId, on_date: onDate, kept },
              { onConflict: 'experiment_id,on_date' })
      .select().single();
    if (error) { say(error.message); return; }
    setExperimentLogs((p) => [...p.filter((l) => !(l.experiment_id === experimentId && l.on_date === onDate)), data]);
    say(kept ? 'Marked kept' : 'Marked missed');
  };
  ...
  return (
    <Ctx.Provider value={{
      ...
      loans, loanPayments, healthDays, experiments, experimentLogs, syncKeys,
      ...
      saveHealthDay, addExperiment, deleteExperiment, logExperiment,
      makeSyncKey, revokeSyncKey,
